...
output_file="/group/mlr-lab/Ryan_Jepson/Tuckute_2024preprint/results/mean_response/ARCV_top_20_no_thresh.csv"
rm -rf $output_file
...
for subj in $f; do
	...
	for area in L_language_parcels wholebrain LR_MD_parcels wholebrain_minus_L_language_parcels MD_not_language_parcels; do
		...
		for parcel_image in $parcel_dir/$area/*bin*.nii; do
			...
			mean_signal_So=$(fslstats $subj_dir/$subj/langlocSN/spmT_0007.nii -k $top10_dir/${subj}/langlocSN/ARCV/${area_top_20_percent}/SMinusN_even_${parcel_name}_top20_percent_mask.nii -M)

			# Use fslstats to extract mean signal S>r (even run) within ROI (odd run)
			mean_signal_Se=$(fslstats $subj_dir/$subj/langlocSN/spmT_0008.nii -k $top10_dir/${subj}/langlocSN/ARCV/${area_top_20_percent}/SMinusN_odd_${parcel_name}_top20_percent_mask.nii -M)

			# Use fslstats to extract mean signal N>r (odd run) within ROI (even run)
			mean_signal_No=$(fslstats $subj_dir/$subj/langlocSN/spmT_0009.nii -k $top10_dir/${subj}/langlocSN/ARCV/${area_top_20_percent}/SMinusN_even_${parcel_name}_top20_percent_mask.nii -M)
		
			# Use fslstats to extract mean signal N>r (even run) within all ROIs (odd run)
			mean_signal_Ne=$(fslstats $subj_dir/$subj/langlocSN/spmT_0010.nii -k $top10_dir/${subj}/langlocSN/ARCV/${area_top_20_percent}/SMinusN_odd_${parcel_name}_top20_percent_mask.nii -M)
