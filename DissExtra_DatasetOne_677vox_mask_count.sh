...
output_sheet="/group/mlr-lab/Ryan_Jepson/Tuckute_2024preprint/results/voxel_counting/voxels_ARCV_top_677.csv"
...
for subj in $f; do
    ...
    for area in wholebrain wholebrain_minus_L_language_parcels; do

        area_no_thresh="${area}_no_thresh"
        area_no_thresh_top_vox="${area}_no_thresh_top_vox"
        ...
        # NB. when area = wholebrain, the 'for' loop runs once despite no .nii files in $roi_dir/wholebrain dir
            ...
            for run in even odd; do
                ...
                count_stats=$(fslstats $mask_dir/$subj/langlocSN/ARCV/$area_no_thresh_top_vox/SMinusN_${run}_${parcel_name}_top${top_n_voxels}_voxels.nii -V)
                read voxel_count cluster_volume <<< $count_stats

                intensity_stats=$(fslstats $mask_dir/$subj/langlocSN/ARCV/$area_no_thresh_top_vox/SMinusN_${run}_${parcel_name}_top${top_n_voxels}_voxels.nii -R)
