% Author: RJ
% Calculating Dice within-subject, even- odd-run similarity, in all top 20% masks, across all subj
...
area_names_top_20_percent = strcat(area_names, '_top_20_percent')
...
for subj=1:length(subj_code);
    ...
    for area=1:length(area_names_top_20_percent);

        % Create list of top 10% masks/parcels from directory, based on current area iteration
        mask_dir = dir(['/group/mlr-lab/Ryan_Jepson/Tuckute_2024preprint/data/top_10_percent_no_thresh/', subj_code{subj}, '/langlocSN/ARCV/', area_names_top_20_percent{area}, '/']);
        mask_names = fullfile({mask_dir.name});
        mask_names = mask_names(~ismember(mask_names, {'.', '..'}));
        mask_names = mask_names(contains(mask_names, '_top20_percent_mask.nii')); % Extracts all top10% masks
        mask_names = mask_names(contains(mask_names, 'SMinusN_even')); % Extracts only even masks, so we have one mask on loop
        mask_names = extractBefore(mask_names,'_top20_percent_mask.nii');
        mask_names = extractAfter(mask_names, 'SMinusN_even_');

        for parcel=1:length(mask_names);

            % Load binary masks
            image1 = niftiread(['/group/mlr-lab/Ryan_Jepson/Tuckute_2024preprint/data/top_10_percent_no_thresh/', subj_code{subj}, '/langlocSN/ARCV/', area_names_top_20_percent{area}, '/SMinusN_even_', mask_names{parcel}, '_top20_percent_mask.nii']);
            image2 = niftiread(['/group/mlr-lab/Ryan_Jepson/Tuckute_2024preprint/data/top_10_percent_no_thresh/', subj_code{subj}, '/langlocSN/ARCV/', area_names_top_20_percent{area}, '/SMinusN_odd_', mask_names{parcel}, '_top20_percent_mask.nii']);
...
writetable(T, '/group/mlr-lab/Ryan_Jepson/Tuckute_2024preprint/results/Dice_analysis/ARCV_Dice_ROI_EvO_top20.csv', 'Delimiter',',','QuoteStrings',true);